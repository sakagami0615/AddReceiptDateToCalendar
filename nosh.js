...
function noshAddReceiptDateToCalendar() {
    ...
    noshAddReceiptDateToCalendarRunner(TARGET_QUERY, SEARCH_NUM, MOVE_ARCHIVE);
...
//--------------------------------------------------------------------------------
function noshExtractDateYearDayTime(date_str) {

    // 不要な文字の削除
    date_str = date_str.replace('指定なし', '').trim();
    ...
    let extract_day = date_str;
    let extract_label = null;

    // 空白分割し、日付と時間を分ける
    // 分割できない場合は、日付のみ返し、時間はnullとする
    let split_item = date_str.split(" ");
    if (split_item.length >= 2) {
        extract_day = split_item[0];
        extract_label = split_item[1];
    }

    return [extract_day, extract_label];
}
...
// 時間のラベル情報を数値情報に変換する
//--------------------------------------------------------------------------------
function noshTimeLabel2Value(label) {
...
function noshExtractReceiptData(body) {
    ...
    // 「お届け予定日時」の日付/時間を取得
    let day_time = noshExtractDateYearDayTime(receipt_lines[date_index + 1]);

    // 時間ラベルを数値に変換
    time_values = noshTimeLabel2Value(day_time[1]);
...
// メイン処理関数
//--------------------------------------------------------------------------------
function noshAddReceiptDateToCalendarRunner(query, max_num, is_archive) {
    ...
    GmailApp.search(query, 0, max_num + 1).forEach(function(thread) {
        ...
        thread.getMessages().forEach(function (message) {
            ...
            // メッセージをconsoleに表示
            let console_message = "◆Nosh Receipt Number " + (count + 1) + "\n";
